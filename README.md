...
| 功能 | 說明 |
| :--- | :--- |
| **每日挑戰** | 以日期為種子（`Core.dailySeed(todayKey(), 8)`）→ 全球同一題、8 色固定；首頁顯示「今天已完成：31 步 ⭐⭐」，完成後寫入當天紀錄 |
| **分享成績** | 結算畫面「📋 分享成績」：`navigator.share` 優先，其次複製到剪貼簿；文字含模式／步數／最佳步數／星等與網址 |
| **🔍 透視** | 盲眼模式下每局 1 次，3 秒內顯示所有顏色（`color-hidden` 全部解除），用完會擋下並說明 |
| **❄️ 上鎖管（地獄模式）** | 開局上鎖一管：**只能倒出、不能倒入**，8 秒後自動解鎖（完成任一管也會提早解鎖） |
| **地獄模式開局更緊** | 12 色的空管由 3 個降為 **2 個**（總管數 15 → 14） |
| **🎁 獎勵空管** | 每完成 3 管獲得 1 個額外空管（每局最多 2 次），補回地獄模式少給的空管 |

> **設計上修正過一次**：原本「凍結」是整管不能倒進也不能倒出，但那樣等於把該管的顏色移出遊戲 ——
> 12 色盤面常常直接無解（實測照合法步走 300 步仍無法完成任何一管）。改成「只鎖倒入方向」＋
> **用真實時間解鎖**（不是步數，玩家永遠等得到），並且開局只給 2 個空管來達到同樣的壓迫感。

## 遊戲性：P2（手感 juice）

- **倒水音高隨水位**：倒了幾顆就響幾聲，音高由來源管水位決定（低水位 → 低音），耳朵能聽出「這管快空了」；原本是固定 5 聲隨機音高，與畫面無關。
- **露出高亮**：下層顏色因為上層被移走而「露出來」時，該色塊會有 0.7 秒的亮起動畫（盲眼模式最重要的回饋）。
- **觸覺回饋**：選取 8ms、錯誤 28ms、完成一管 18ms（`navigator.vibrate`，並尊重 `prefers-reduced-motion`）。
